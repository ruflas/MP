...
public abstract class Hero extends Character implements Tesorero{
	...
	public int defend(int impacts) {
		...
		if (impacts > 0) {
			...
			System.out.printf(this.getName() + " cannot block %d impacts%s", impacts,
					(isAlive() ? "\n" : " and dies\n"));
		} else {
			System.out.printf("The hero %s blocks the attack\n", this.getName());
		}
	...
	public void combat(Character c, Game currentGame) { // attacks to c and c defends itself
		...
		if (!c.isAlive() && c instanceof Tesorero) {
			this.setMonedas(this.getMonedas()+((Tesorero)c).getMonedas());
			((Tesorero) c).setMonedas(0);
