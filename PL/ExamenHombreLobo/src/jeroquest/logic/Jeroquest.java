...
import jeroquest.units.Werewolf;
...
public class Jeroquest {
	...
	private void nextRound() {
		if(currentGame.getCurrentRound()%3==0) {
			for(int i = 0; i<currentGame.getCharacters().length; i++) {
				Character c = currentGame.getCharacters()[i];
				if(c.isAlive() && c instanceof Werewolf) ((Werewolf)c).regeneracion();
			}
		}
	...
	private void resolveRound() {
		...
		if(currentGame.getCurrentRound()%3==0) {
			for(int i = 0; i<currentGame.getCharacters().length; i++) {
				Character c = currentGame.getCharacters()[i];
				int r = Dice.roll(2);
				if(r==2)if(c instanceof Werewolf) ((Werewolf)c).transformacion(true);
			}
		}
		else if((currentGame.getCurrentRound()-1)%3==0){
			for(int i = 0; i<currentGame.getCharacters().length; i++) {
				Character c = currentGame.getCharacters()[i];
				if(c instanceof Werewolf) ((Werewolf)c).transformacion(false);
			}
		}
