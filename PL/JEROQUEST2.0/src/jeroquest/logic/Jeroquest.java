...
import jeroquest.units.Mosquiman;
...
public class Jeroquest {
	...
	private void nextRound() {
		...
		for(int i = 0; i<currentGame.getCharacters().length; i++) {
			...
			if(c.isAlive() && c instanceof Mosquiman && ((Mosquiman)c).getEnergy()==3) { //At the end of the round all the mosquimans who are alive and have the max level of energy ->
				if(c.getBody()<c.getBodyInitial()) ((Mosquiman)c).moreStrength(); // -> If they have they body lower than the initial, they call the moreStrength() function to increase his body.
			}
		}
	}
