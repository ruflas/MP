...
public class Jeroquest {
	...
	public void showUndamagedWasps() {
		for(int i = 0; i<currentGame.getCharacters().length; i++) {
			if(currentGame.getCharacters()[i] instanceof Swarm) {
				Character c = currentGame.getCharacters()[i];
				DynamicVectorCharacters waspv = ((Swarm) c).getWasps();
				for(int j=0; j<waspv.length();j++) {
					System.out.println(waspv.get(j).toString());
				}
			}
		}
	}
